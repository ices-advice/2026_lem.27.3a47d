...
# Note: this needs to be updated, so that the same
# function is used for both standard and retro runs
...
plot.surbar <- function(x, stock, psim, psim.s, psim.f, psim.r, res, y1, y2, 
                        a1, a2, type, pdf, filename, filename1, filename2)
{
  ...
  if (type == "sum.line")
  {
    ...
    legend(legend = c("NLS estimate", "Bootstrap mean", "Bootstrap median", "90% CI"), x = "topleft",
  ...
  } else if (type == "res.line")
  {
    ...
    for (k in 1:numk)
    {
      ...
      if (!pdf) { if(k==1)savePlot(filename1,type = "tiff") else savePlot(filename2,type = "tiff")
      dev.off()}
    }
  ...
  } else if (type == "catch.curve")
  {
    for (k in 1:numk)
    {
      ...
      # Remove NAs (to ensure cohort year printed for all lines)
      cc <- cc[!is.na(cc$index),]
      ...
      for (j in cc.cohorts)
      {
        jj <- jj + 1
        cc0 <- cc[cc$cohort == j,]
        lines(cc0$year, log(cc0$index), type = "l", lty =1, lwd=2 , pch = -1,
              col = turbo(n = length(cc.cohorts))[jj])
        if (length(cc0$index) > 1)
        {
          text(x = cc0$year[1]-0.2, y = log(cc0$index)[1] + 0.01, cc0$cohort[1], cex = 0.7)
        }
      }		
    }
  } else if (type == "log.by.cohort")
  {
    ...
    for (k in 1:numk)
    {
      ...
      for (j in lbc.ages)
      {
        jj <- jj + 1
        lbc0 <- lbc[lbc$age == j,]
        lines(lbc0$cohort, lbc0$index, type = "l", lty = 1,lwd=2, 
